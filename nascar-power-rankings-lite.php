<?php
/*
Plugin Name: NASCAR Power Rankings Lite
...
function NASCARPRLPageURL() {
 $NASCARPRLpageURL = 'http';
 $NASCARPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NASCARPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NASCARPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NASCARPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_nascarsprlstats() 
{
?>
<h2>NASCAR Power Rankings Lite</h2>
<?php nascarsprl_stats(); ?>
<?php
}

function nascarsprlstats_install()
{
register_sidebar_widget(__('NASCAR Power Rankings Lite'), 'widget_nascarsprlstats'); 
}
add_action("plugins_loaded", "nascarsprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'nascarsprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'nascarsprl_stats_remove' );

function nascarsprl_stats_install() 
{
...
add_option("nascarsprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("nascarsprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("nascarsprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NASCAR Power Rankings Installation", "Hi\n\nNASCAR Power Rankings Activated at \n\n".NASCARPRLPageURL()."\n\nLITE NASCAR Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function nascarsprl_stats_remove() 
{
/* Deletes the database field */
delete_option('nascarsprl_scroll_text_color');
delete_option('nascarsprl_scroll_text_color1');
delete_option('nascarsprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'nascarsprl_stats_admin_menu');

function nascarsprl_stats_admin_menu() {

add_options_page('NASCAR Power Rankings Lite', 'NASCAR Power Rankings Lite Settings', 'administrator', 'nascar-power-rankings-lite.php', 'nascarsprl_stats_plugin_page');
}
}
function nascarsprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('nascarsprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('nascarsprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('nascarsprl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/nascar_power_rankings_magpie_lite/nascar_power_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = nascarsprl_fetch_rss( $url );
...
function nascarsprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NASCAR Power Rankings Options Page</h2>
...
<p>
<?php echo nascarsprl_stats(); ?>
</p>
<?php echo $mydisplay; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want to Customize Scroller and Display More Drivers:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to customize your scroller's colors and display more driver ranking items.
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NASCAR Power Rankings Scroller package') ?></h5>
    <form id="DownloadForm" name="DownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/nascar-power-rankings.zip', 'Download'); return false;">
      </label>
    </form>
    
    
    <br />
    <a href="http://www.ibet.ws/download/nascar-power-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
